...
<div class="regist">
	...
	<div>
		<label>ID </label>
		<input type="textfiled" name="id" value="${login.id}">
	</div>
	<div>
		<label>PW </label>
		<input type="password" name="pw" value="${login.pw}">
	</div>
	<div>
		<label>성별</label><!-- 수정을 하려고 하는데 자동으로 체크 되어 있게 하려면 db의 정보를 받아와서 비교후에 설정을 준다. -->
		<input type="radio" name="gender" value="male" 
			<c:if test="${login.gender=='male' }">checked="checked"</c:if>>남</input>
		<input type="radio" name="gender" value="female"
			<c:if test="${login.gender=='female' }">checked="checked"</c:if>>여</input>
	</div>
	<div>
